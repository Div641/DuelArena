...
export default function Navbar({ 
  ...
  // Check if rendered in the Arena workspace page (always black background)
  const isArena = isAuthenticated && isAuthPage;

  // Custom visual classes for light mode compatibility on black background
  const headerBorderClass = isArena 
    ? 'border-b-3 border-slate-800 bg-black/80' 
    : 'border-b-3 border-black dark:border-white bg-transparent';
  
  const logoTextClass = isArena 
    ? 'text-white' 
    : 'text-black dark:text-white';
  
  const logoBadgeClass = isArena
    ? 'bg-[#5CA1E6] text-black border-2 border-white shadow-[2px_2px_0px_0px_#fff]'
    : 'bg-[#5CA1E6] text-black border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_#000] dark:shadow-[2px_2px_0px_0px_#fff]';

  const separatorClass = isArena
    ? 'text-slate-600'
    : 'text-black dark:text-white';

  const docButtonClass = isArena
    ? 'flex items-center gap-1 bg-[#1E1E22] text-white px-3 py-1.5 border-[2.5px] border-slate-700 shadow-[3px_3px_0px_0px_#fff] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[5px_5px_0px_0px_#fff] cursor-pointer transition-all duration-150'
  ...
  const gitButtonClass = isArena
    ? 'flex items-center gap-1 bg-[#1E1E22] text-white px-3 py-1.5 border-[2.5px] border-slate-700 shadow-[3px_3px_0px_0px_#fff] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[5px_5px_0px_0px_#fff] cursor-pointer transition-all duration-150'
    : 'flex items-center gap-1 bg-white dark:bg-slate-900 text-black dark:text-white px-3 py-1.5 border-[2.5px] border-black dark:border-white shadow-[3px_3px_0px_0px_#000] dark:shadow-[3px_3px_0px_0px_#fff] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[5px_5px_0px_0px_#000] dark:hover:shadow-[5px_5px_0px_0px_#fff] cursor-pointer transition-all duration-150';

  const homeButtonClass = isArena
    ? 'px-4 py-2 text-xs font-black uppercase tracking-wider bg-white text-black border-[3px] border-white shadow-[4px_4px_0px_0px_#5CA1E6] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#5CA1E6] cursor-pointer transition-all duration-150'
    : 'px-4 py-2 text-xs font-black uppercase tracking-wider bg-white dark:bg-slate-900 text-black dark:text-white border-[3px] border-black dark:border-white shadow-[4px_4px_0px_0px_#000] dark:shadow-[4px_4px_0px_0px_#fff] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#000] dark:hover:shadow-[6px_6px_0px_0px_#fff] cursor-pointer transition-all duration-150';

  const themeButtonClass = isArena
    ? 'p-2 bg-[#1E1E22] border-[3px] border-slate-700 shadow-[4px_4px_0px_0px_#fff] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#fff] text-white cursor-pointer transition-all duration-150'
  ...
  const userButtonClass = isArena
    ? 'p-2 bg-[#1E1E22] border-[3px] border-slate-700 shadow-[4px_4px_0px_0px_#fff] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#fff] text-white cursor-pointer transition-all duration-150'
    : 'p-2 bg-white dark:bg-slate-900 border-[3px] border-black dark:border-white shadow-[4px_4px_0px_0px_#000] dark:shadow-[4px_4px_0px_0px_#fff] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#000] dark:hover:shadow-[6px_6px_0px_0px_#fff] text-black dark:text-white cursor-pointer transition-all duration-150';

  return (
    <header className={`sticky top-0 z-40 w-full transition-all duration-300 ${headerBorderClass}`}>
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        
        {/* Left: DuelArena logo with Neo-Brutalist styling */}
        <div className="flex items-center gap-2">
          <span className={`font-display text-2xl font-black uppercase tracking-tighter select-none ${logoTextClass}`}>
            DuelArena
          </span>
          <span className={`px-2 py-0.5 text-[9px] font-black uppercase tracking-wider ${logoBadgeClass}`}>
            v1.0
          </span>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-4 text-xs font-bold uppercase tracking-wider">
          ...
          <span className={`font-extrabold ${separatorClass}`}>|</span>
          <a 
